...
The site is in an early/placeholder state: `index.html` is a self-contained "GitHub Pages Home" page (all CSS inlined in a `<style>` block, no external assets or dependencies). The README describes it as a **Landing Page for Github Pages**.
...
```bash
./runlocal.sh          # serves on http://localhost:8000 (pass a port to override)
python3 -m http.server 8000   # equivalent, if you prefer to run it directly
```
